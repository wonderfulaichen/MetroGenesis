...
import com.metrogenesis.minecolonies.api.colony.managers.interfaces.IEventDescriptionManager;
...
import static com.metrogenesis.minecolonies.api.util.constant.ColonyConstants.MAX_COLONY_EVENTS;
...
public class EventDescriptionManager implements IEventDescriptionManager
{
    ...
    public void computeNews()
    {
        ...
        for (final Object2IntMap.Entry<String> entry : summaries.object2IntEntrySet())
        {
            if (builder == null)
            {
                builder = MessageUtils.format(Component.translatable("com.metrogenesis.minecolonies.core.event.summary.prefix")).append(Component.translatable(entry.getKey(), entry.getIntValue()));
            }
            else
            {
