package com.metrogenesis.minecolonies.core.network.messages.server;
...
import com.metrogenesis.minecolonies.core.network.messages.client.OpenCantFoundColonyWarningMessage;
import com.metrogenesis.minecolonies.core.network.messages.client.OpenColonyFoundingCovenantMessage;
import com.metrogenesis.minecolonies.core.network.messages.client.OpenDeleteAbandonColonyMessage;
import com.metrogenesis.minecolonies.core.network.messages.client.OpenReactivateColonyMessage;
...
import static com.metrogenesis.minecolonies.api.util.constant.BuildingConstants.DEACTIVATED;
import static com.metrogenesis.minecolonies.api.util.constant.TranslationConstants.HUT_BLOCK_MISSING_BUILDING;
...
public class GetColonyInfoMessage implements IMessage
{
    ...
    public void onExecute(final NetworkEvent.Context ctxIn, final boolean isLogicalServer)
    {
        ...
        if (IColonyManager.getInstance().isFarEnoughFromColonies(world, pos))
        {
            ...
            {
                Network.getNetwork().sendToPlayer(new OpenCantFoundColonyWarningMessage(Component.translatable("com.metrogenesis.minecolonies.core.founding.tooclosetospawn", (int) (MineColonies.getConfig().getServer().minDistanceFromWorldSpawn.get() - spawnDistance)), pos, true), sender);
            }
            else if (spawnDistance > MineColonies.getConfig().getServer().maxDistanceFromWorldSpawn.get())
            {
                Network.getNetwork().sendToPlayer(new OpenCantFoundColonyWarningMessage(Component.translatable("com.metrogenesis.minecolonies.core.founding.toofarfromspawn", (int) (spawnDistance - MineColonies.getConfig().getServer().maxDistanceFromWorldSpawn.get())), pos, true), sender);
            }
        ...
        else
        {
            ...
            Network.getNetwork().sendToPlayer(new OpenCantFoundColonyWarningMessage(Component.translatable("com.metrogenesis.minecolonies.core.founding.tooclosetocolony", Math.max(100, blockRange - distance)), pos, false), sender);
        }
    }
}
