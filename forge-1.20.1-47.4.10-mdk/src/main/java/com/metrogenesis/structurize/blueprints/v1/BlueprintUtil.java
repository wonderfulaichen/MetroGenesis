package com.metrogenesis.structurize.blueprints.v1;
...
public class BlueprintUtil
{
    ...
    public static List<BlockState> fixPalette(final int oldDataVersion, final ListTag paletteTag)
    {
        ...
        for (short i = 0; i < paletteSize; i++)
        {
            ...
            try
            {
                ...
                // ★ 兼容：domum_ornamentum → metrogenesis_domum（蓝图存的是旧 modid）
                if (!state.isAir())
                {
                    palette.add(i, state);
                }
                else
                {
                    ...
                    if (name.startsWith("domum_ornamentum:"))
                    {
                        fixedNbt.putString("Name", name.replace("domum_ornamentum", "metrogenesis_domum"));
                        BlockState remapped = NbtUtils.readBlockState(BuiltInRegistries.BLOCK.asLookup(), fixedNbt);
                        if (!remapped.isAir())
                        {
        ...
        if (name.contains(MOD_ID))
        {
            if (name.contains("blockshingle_"))
            {
                ...
                oldBlockState.putString("Name",
                  "metrogenesis_structurize:clay_" + (split.length > 2 ? split[1] + "_" + split[2] : split[1]) + "_shingle");
            }
            else if (name.contains("blockshingleslab"))
            {
                oldBlockState.putString("Name", "metrogenesis_structurize:clay_shingle_slab");
            }
            else if (name.contains("blocktimberframe"))
            {
                final String[] split = name.split(":")[1].split("_");
                String output = "metrogenesis_structurize:" + (split.length > 3 ? split[3] : split[2]) + "_"
            ...
            {
                oldBlockState.putString("Name",
                  "metrogenesis_structurize:" + oldBlockState.getCompound("Properties").getString("variant") + "_blockpaperwall");
            }
        }
    }
    ...
    public static CompoundTag[] fixTileEntities(final int oldDataVersion, final ListTag tileEntitiesTag)
    {
        ...
        for (short i = 0; i < tileEntities.length; i++)
        {
            ...
            try
            {
                ...
                // ★ 兼容：蓝图里存的是老 modid，替换为新 modid
                if (id.startsWith("domum_ornamentum:"))
                {
                    nbt.putString("id", id.replace("domum_ornamentum", "metrogenesis_domum"));

                    // ★ 递归修复 textureData 内部的方块引用（如 domum_ornamentum:block/shingle → metrogenesis_domum:block/shingle）
                    if (nbt.contains("textureData", net.minecraft.nbt.Tag.TAG_COMPOUND))
                    {
                        final CompoundTag textureData = nbt.getCompound("textureData");
                        final CompoundTag fixedTextureData = new CompoundTag();
                        for (final String key : textureData.getAllKeys())
                        {
                            final String rawValue = textureData.getString(key);
                            if (rawValue.startsWith("domum_ornamentum:"))
                            {
                                fixedTextureData.putString(key, rawValue.replace("domum_ornamentum", "metrogenesis_domum"));
                            }
                            else
                            {
                                fixedTextureData.putString(key, rawValue);
                            }
                        }
                        nbt.put("textureData", fixedTextureData);
                    }
    ...
    private static java.util.Map<String, com.mojang.serialization.Dynamic<?>> getFlowerPotMap() {
        try {
            Field f = net.minecraft.util.datafix.fixes.ChunkPalettedStorageFix.class.getDeclaredField("FLOWER_POT_MAP");
            f.setAccessible(true);
            @SuppressWarnings("unchecked")
            java.util.Map<String, com.mojang.serialization.Dynamic<?>> map = (java.util.Map<String, com.mojang.serialization.Dynamic<?>>) f.get(null);
            return map;
        } catch (Exception e) { return java.util.Map.of(); }
    }

    private static java.util.Map<String, com.mojang.serialization.Dynamic<?>> getNoteBlockMap() {
        try {
            Field f = net.minecraft.util.datafix.fixes.ChunkPalettedStorageFix.class.getDeclaredField("NOTE_BLOCK_MAP");
            f.setAccessible(true);
            @SuppressWarnings("unchecked")
            java.util.Map<String, com.mojang.serialization.Dynamic<?>> map = (java.util.Map<String, com.mojang.serialization.Dynamic<?>>) f.get(null);
            return map;
        ...
        for (short i = 0; i < oldSize; i++)
        {
            ...
            if (bs.getBlock() == Blocks.POTTED_CACTUS) // flower pot fix
            {
                teToBlockStateFix(palette, blocks, tileEntities, i, teCompound -> {
                    ...
                    return (CompoundTag) getFlowerPotMap()
                                           .getOrDefault(type, getFlowerPotMap().get("minecraft:air0"))
                                           .getValue();
                });
            }
            else if (bs.getBlock() == Blocks.NOTE_BLOCK) // note block fix
            {
                teToBlockStateFix(palette, blocks, tileEntities, i, teCompound -> {
                    ...
                    return (CompoundTag) getNoteBlockMap()
                                           .getOrDefault(type, getNoteBlockMap().get("false0"))
                                           .getValue();
                });
            }
        }
