...
import com.metrogenesis.structurize.blocks.interfaces.ILeveledBlueprintAnchorBlock;
import com.metrogenesis.structurize.blocks.interfaces.INamedBlueprintAnchorBlock;
import com.metrogenesis.structurize.blocks.interfaces.IRequirementsBlueprintAnchorBlock;
import com.metrogenesis.structurize.blueprints.v1.Blueprint;
import com.metrogenesis.structurize.blueprints.v1.BlueprintTagUtils;
import com.metrogenesis.structurize.network.messages.BuildToolPlacementMessage;
import com.metrogenesis.structurize.network.messages.SyncPreviewCacheToServer;
...
import static com.metrogenesis.structurize.api.util.constant.GUIConstants.DEFAULT_ICON;
import static com.metrogenesis.structurize.api.util.constant.WindowConstants.BUILD_TOOL_RESOURCE_SUFFIX;
import static com.metrogenesis.structurize.api.util.constant.WindowConstants.BUTTON_CONFIRM;
...
public final class WindowExtendedBuildTool extends AbstractBlueprintManipulationWindow
{
    ...
    /**
     * Cached construction position, used for re-init retry in onOpened().
     */
    @Nullable
    ...
    {
        // Retry pack selection: by the time onOpened() runs (first tick),
        // the async IO thread has likely finished discovering structure packs.
        if (StringUtils.isBlank(currentStructurePack))
        {
            ...
            if (selectedPack != null)
            {
                // Async discovery completed; re-initialize the UI state.
                this.init(this.groundstyle, this.initPos);
            }
        }
        ...
        if (categoryFutures != null && categoryFutures.isDone())
        {
            ...
            try
            {
                ...
                for (final StructurePacks.Category category : categoryFutures.get())
                {
                    ...
                    {
                        blueprintsAtDepth.put(id, StructurePacks.getBlueprintsFuture(currentStructurePack, id));
                    }
                    else
                    {
