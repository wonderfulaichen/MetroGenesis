...
import com.metrogenesis.minecolonies.api.research.*;
import com.metrogenesis.minecolonies.api.research.requirements.BuildingAlternatesResearchRequirement;
import com.metrogenesis.minecolonies.api.research.requirements.BuildingResearchRequirement;
import com.metrogenesis.minecolonies.api.research.util.ResearchState;
...
import com.metrogenesis.minecolonies.core.client.gui.blockui.RotatingItemIcon;
import com.metrogenesis.minecolonies.core.client.gui.modules.building.UniversityModuleWindow;
import com.metrogenesis.minecolonies.core.network.messages.server.colony.building.university.TryResearchMessage;
import com.metrogenesis.minecolonies.core.research.GlobalResearchEffect;
...
public class WindowResearchTree extends AbstractWindowSkeleton
{
    ...
    private void drawTreeBackground(final ZoomDragView view, final int maxHeight)
    {
        ...
        for (int i = 1; i <= MAX_DEPTH; i++)
        {
            ...
            {
                timeLabel.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.gui.research.tier.header.unrestricted",
            ...
            {
                timeLabel.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.gui.research.tier.header",
                  (i > building.getBuildingMaxLevel()) ? building.getBuildingMaxLevel() : i,
                  IGlobalResearchTree.getInstance().getBranchData(branch).getHoursTime(i)));
      ...
      final int progress)
    {
        ...
        switch (state)
        {
            ...
            case TOO_MANY_PROGRESS:
                ...
                tooMany1.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.research.toomanyinprogress.1"));
                ...
                tooMany2.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.research.toomanyinprogress.2"));
        ...
        if (state != ResearchButtonState.FINISHED && state != ResearchButtonState.IN_PROGRESS)
        {
            ...
            for (final IResearchCost cost : research.getCostList())
            {
                hoverPaneBuilder.paragraphBreak()
                  ...
                  .append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.limit.requirement", cost.getCount(), cost.getTranslatedName()));
            ...
            {
                hoverPaneBuilder.paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.requirement.university.level", Math.min(research.getDepth(), this.building.getBuildingMaxLevel())));
            }
            if (research.getDepth() == MAX_DEPTH && branchType != ResearchBranchType.UNLOCKABLES)
            {
                if (hasMax)
                {
                    hoverPaneBuilder.paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_UNFULFILLED);
                }
                else
                {
                    hoverPaneBuilder.paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.limit.onemaxperbranch")).color(COLOR_TEXT_FULFILLED);
                }
            }
        }
        if (research.isImmutable())
        {
            hoverPaneBuilder.paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.limit.immutable")).color(COLOR_TEXT_UNFULFILLED);
        }
    ...
    private void drawResearchTexts(final ZoomDragView view, final int offsetX, final int offsetY, final IGlobalResearch research, final ResearchButtonState state, final int progress)
    {
        ...
        if (state == ResearchButtonState.IN_PROGRESS)
        {
            ...
            progressLabel.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.gui.research.time", timeRemaining));
    ...
    private void drawUndoProgressButton(final Button parent)
    {
        ...
        undoText.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.undo.progress"));
        undoText.disable();
        parent.getParent().addChild(undoText);
        PaneBuilders.tooltipBuilder().hoverPane(undoButton).append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.undo.progress.tooltip")).color(COLOR_TEXT_UNFULFILLED).bold().build();
    }
    ...
    private void drawUndoCompleteButton(final Button parent)
    {
        ...
        final AbstractTextBuilder.TooltipBuilder undoTipBuilder = PaneBuilders.tooltipBuilder().hoverPane(undoButton)
                                  .append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.undo.remove.tooltip")).bold().color(COLOR_TEXT_UNFULFILLED);
        undoText.setSize(BUTTON_LENGTH, BUTTON_HEIGHT);
        ...
        if (!missingItems.isEmpty())
        {
            ...
            undoText.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.research.notenoughresources"));
            for (ItemStorage cost : missingItems)
            {
                undoTipBuilder.paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.requirement.research",
        ...
        else
        {
            undoButton.setImage(new ResourceLocation(Constants.MOD_ID, MEDIUM_SIZED_BUTTON_RES), false);
            undoButton.setID("undo:" + parent.getID());
            undoText.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.undo.remove"));
        }
        undoText.disable();
        parent.getParent().addChild(undoButton);
        parent.getParent().addChild(undoText);
      ...
      final ResearchButtonState state)
    {
        if (research.isImmutable() && state != ResearchButtonState.FINISHED)
        {
            ...
            PaneBuilders.tooltipBuilder().hoverPane(immutIcon).paragraphBreak().append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.limit.immutable"))
        ...
        else
        {
            ...
            else
            {
                if (IGlobalResearchTree.getInstance().getResearch(branch, parentResearch).hasOnlyChild())
                {
                    ...
                    orLabel.setText(Component.translatable("com.metrogenesis.minecolonies.coremod.research.research.or"));
                    orLabel.setPosition(offsetX + INITIAL_X_OFFSET, offsetY + TEXT_Y_OFFSET);
                    view.addChild(orLabel);
                    PaneBuilders.tooltipBuilder().hoverPane(orLabel).append(Component.translatable("com.metrogenesis.minecolonies.coremod.research.research.or.tooltip")).build();
