...
import com.metrogenesis.minecolonies.api.colony.interactionhandling.AbstractInteractionResponseHandler;
...
import com.metrogenesis.minecolonies.core.client.gui.citizen.MainWindowCitizen;
import com.metrogenesis.minecolonies.core.network.messages.server.colony.InteractionResponse;
...
public abstract class ServerCitizenInteraction extends AbstractInteractionResponseHandler
{
    ...
    public boolean onClientResponseTriggered(final int responseId, final Player player, final ICitizenDataView data, final BOWindow window)
    {
        final Component response = getPossibleResponses().get(responseId);
        tryHandleIgnoreResponse(response, player);
        if (((TranslatableContents) getPossibleResponses().get(responseId).getContents()).getKey().equals("com.metrogenesis.minecolonies.coremod.gui.chat.skipchitchat"))
        {
